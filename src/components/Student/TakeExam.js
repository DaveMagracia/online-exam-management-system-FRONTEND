...
export default function TakeExam() {
            ...
            .then((res) => {
               ...
               if (res.data.exam.status === "closed") {
                  ...
                  if (currentTime > endDateMinusLimit && currentTime < dateTo) {
                     ...
                     setTimeLimitString(
                        `${newTimeLimit} ${newTimeLimit === 1 ? "minute" : "minutes"} (originally ${
                     ...
                     setEndTime(new Date(currentTime.getTime() + res.data.exam.time_limit * 60000)); //when the exam will end
   ...
   async function submitExam(event) {
      ...
      const results_ = questions.map((val, i) => {
         ...
         if (isCorrect) totalScore += val.points;
   ...
   React.useEffect(() => {
      ...
      //this is kind of an infinite loop because for every time a state changes,
      //the whole component re-renders, also causing this useEffect to run

      function stopTimer() {
         clearTimeout(intervalId);
         console.log("submit");
         //automatically stop and submit the exam at this point
         submitExam();
      }
      ...
      const intervalId = setInterval(() => {
         setTimeLimit((prevVal) => {
            ...
            formatTime(newVal);
            return newVal <= 0 ? stopTimer() : newVal; //stop timer if it is lt or equal to 0
         });
      }, 1000);
                     ...
                     <>
                        ...
                        <div ref={boxRef} className={`${css.collapsible} border p-4`}>
                           ...
                           <div className={`${css.content}`}>
                              ...
                              <ul
                                 className={`${css.questions_list} list-group list-group-numbered border border-start-0 border-end-0`}>
                                 {questions.map((val, i) => {
                                       ...
                                       return (
                                          <li
                                             ...
                                             <span
                                                ...
                                                {val.points} {val.points === 1 ? "point" : "points"}
                                             </span>
                                          </li>
                                       );
                              ...
                              <form onSubmit={submitExam}>
                                 {questions.length > 0 && (
                                    <div className="card p-5">
                                       ...
                                       <p className="mt-3">{questions[currQuestionNum].question}</p>
